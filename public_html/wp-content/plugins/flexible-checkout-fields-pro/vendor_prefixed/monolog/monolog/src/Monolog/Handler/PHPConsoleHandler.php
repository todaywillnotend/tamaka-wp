...
use FCFProVendor\PhpConsole\Handler;
 ...
 */
class PHPConsoleHandler extends \FCFProVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \FCFProVendor\PhpConsole\Connector $connector = null, $level = \FCFProVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
    ...
    private function initConnector(\FCFProVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \FCFProVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \FCFProVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\FCFProVendor\PhpConsole\Helper::isRegistered()) {
            \FCFProVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \FCFProVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \FCFProVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \FCFProVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \FCFProVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
